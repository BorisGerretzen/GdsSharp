...
public class PathBuilderBase : IPathBuilder
{
    protected readonly Vector2 _initialHeading;
    protected readonly Vector2 _initialPosition;
    protected readonly float _initialWidth;
    protected readonly List<GdsPathSegment> _pathSegments = new();

    public PathBuilderBase(float initialWidth, Vector2? initialPosition = null, Vector2? initialHeading = null)
    {
    ...
    public PathBuilderBase AddPathSegment(Func<float, Vector2> path, Func<float, Vector2> derivative, Func<float, float?>? width = null, int vertices = 10)
    {
        _pathSegments.Add(new GdsPathSegment(path, derivative, width, vertices));
        return this;
    }

    public virtual IPathBuilder Straight(int length, float? widthStart = null, float? widthEnd = null, Func<float, float?>? width = null, int vertices = 2)
    {
    ...
    public virtual PathBuilderBase BendRad(float angle, int radius, Func<float, float?>? width = null, int vertices = 128)
    {
    ...
    public virtual IPathBuilder BendDeg(float angle, int radius, Func<float, float?>? width = null, int vertices = 128)
    {
    ...
    /// <param name="width">(optional) Function that provides a width for t on the interval [0,1].</param>
    /// <param name="vertices">(optional) Number of vertices used for the path.</param>
    public virtual IPathBuilder Bezier(Action<BezierBuilder> build, Func<float, float?>? width = null, int vertices = 128)
    {
    ...
    public virtual IEnumerable<GdsElement> Build(int maxVertices = 200)
    {
        if (maxVertices < 4)
    ...
    /// </summary>
    /// <returns></returns>
    protected virtual IEnumerable<GdsPoint> GetPolygonPoints()
    {
    ...
    protected virtual List<(GdsPathSegment, List<GdsPathPoint>)> GetPathPoints()
    {
        var points = new List<(GdsPathSegment, List<GdsPathPoint>)>();
