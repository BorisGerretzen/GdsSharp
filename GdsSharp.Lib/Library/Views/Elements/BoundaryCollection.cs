...
public readonly struct BoundaryCollection
{
    public readonly int StructureIndex;
    private readonly GdsLibrary _library;
    
    internal BoundaryCollection(GdsLibrary library, int structureIndex)
    {
        _library = library;
    ...
    public struct Enumerator
    {
        ...
        public BoundaryView Current => new(_lib, _elementIndex + _elementsStartIndex);

        public bool MoveNext()
        {
            while (++_elementIndex < _elementsCount)
            {
                if (_lib.Elements[_elementIndex + _elementsStartIndex].Kind == GdsElementKind.Boundary)
                    return true;
            }
