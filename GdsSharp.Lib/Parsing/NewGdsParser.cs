...
public sealed class NewGdsParser(NewGdsTokenStream tokenStream)
{
    ...
    private void ParseBoundary(IParserConsumer consumer)
    {
        ...
        var points = ArrayPool<GdsPoint>.Shared.Rent(numPoints);
        try
        {
            tokenStream.ReadXy(xyHeader, points.AsSpan(0, numPoints));
            consumer.OnBeginElement(GdsElementKind.Boundary, common);
            consumer.OnBoundary(layer, dataType, points.AsSpan(0, numPoints));
            ConsumePropertiesAndEndElement(consumer);
        }
        finally
        {
            ArrayPool<GdsPoint>.Shared.Return(points);
        }
    }
    ...
    private void ParsePath(IParserConsumer consumer)
    {
        ...
        var pts = ArrayPool<GdsPoint>.Shared.Rent(numPoints);
        try
        {
            tokenStream.ReadXy(xyHeader, pts.AsSpan(0, numPoints));
            consumer.OnBeginElement(GdsElementKind.Path, common);
            consumer.OnPath(layer, dataType, pathType, width, pts.AsSpan(0, numPoints));
            ConsumePropertiesAndEndElement(consumer);
        }
        finally
        {
            ArrayPool<GdsPoint>.Shared.Return(pts);
        }
    }
    ...
    private void ParseSref(IParserConsumer consumer)
    {
        ...
        try
        {
            ...
            consumer.OnSref(structureName, strans, pts.AsSpan(0, numPoints));
            ConsumePropertiesAndEndElement(consumer);
        }
        finally
        {
    ...
    private void ParseAref(IParserConsumer consumer)
    {
        ...
        try
        {
            ...
            consumer.OnAref(targetName, strans, columns, rows, points.AsSpan(0, numPoints));
            ConsumePropertiesAndEndElement(consumer);
        }
        finally
        {
    ...
    private void ParseText(IParserConsumer consumer)
    {
        ...
        try
        {
            ...
            consumer.OnText(layer, textType, pres, pathType, width, strans, points.AsSpan(0, numPoints), text);
            ConsumePropertiesAndEndElement(consumer);
        }
        finally
        {
    ...
    private void ParseBox(IParserConsumer consumer)
    {
        ...
        try
        {
            ...
            consumer.OnBox(layer, boxType, points.AsSpan(0, numPoints));
            ConsumePropertiesAndEndElement(consumer);
        }
        finally
        {
    ...
    private void ParseNode(IParserConsumer consumer)
    {
        ...
        try
        {
            ...
            consumer.OnNode(layer, nodeType, points.AsSpan(0, numPoints));
            ConsumePropertiesAndEndElement(consumer);
        }
        finally
        {
